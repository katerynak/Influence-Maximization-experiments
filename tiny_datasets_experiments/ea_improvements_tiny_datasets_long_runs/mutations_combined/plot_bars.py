...
for out_dir, title, res_dir in zip(out_dirs, titles, res_dirs):
	...
	for k in [2, 3]:
		for y in Y:
			...
			labels = []
			for fv in F_values:
				if "ea_global_activation_mutation" in fv and "ea_local_activation_mutation" in fv:
					label = "activation_mutations"
				elif "local" in fv:
					label = "local_mutations"
				elif "global" in fv:
					label = "global_mutations"
			...
			ax.set(xticks=F_values, xticklabels=labels)
			fig.autofmt_xdate()
